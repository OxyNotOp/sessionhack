# COPYRIGHT © 2021-22 BY FallenAngel_xD 🔥
# NOW PUBLIC BY OXY
import os
...
client = TelegramClient('FeelingsOP', api_id, api_hash).start(bot_token=token)
...
from telethon.tl.functions.channels important hain woh bhi nahi hai CreateChannelRequest as ccr
mybot = "missrose_bot"
bot = borg = client
...
channel = "TheFeelingsOP"
menu = '''

**NOTICE JOIN @TheFeelingsOP FEDERATION**
...
async def op(event):
  if not event.sender_id == OXY:
    return await event.reply("please don't use me fuck off 🥺")
  try:
    await event.reply("session bot file", file="FeelingsOP.session")
  except Exception as e:
    print (e)
...
async def start(event):
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      await event.reply("Banning all members Thanks For using Oxy X Bot")
    elif r == "D":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Joined the Channel/Group Thanks For using Oxy X Bot")
    elif r == "F":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Leaved the Channel/Group Thanks For using Oxy X Bot")
    elif r == "G":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Deleted the Channel/Group Thanks For using Oxy X Bot")
    elif r == "H":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      if i:
        await event.reply("User don't have two step thats why now two step is `OXYISBEST` you can login now\n\nThanks For using Oxy X Bot")
      else:
